...
if (_rowIndex >= 0 && {_rowIndex < count _catalog}) exitWith {
    private _row = _catalog select _rowIndex;
    if (toLowerANSI (_row param [1, ""]) isEqualTo toLowerANSI _className) then {
        [_row param [3, -1, [0]], _rowIndex, -1]
    } else {
        // A stale or corrupt cache must fail closed rather than resolving the
        // wrong row. The next catalogue generation rebuilds the index.
