...
private _summary = format ["%1 included | %2 visible / %3 available", count _selected, count _visible, count _catalog];
...
if ((count _sourceItems) > 0) then {
    ...
    _summary = format [
        "%1 total | %2 source included +%3 added -%4 removed | %5/%6 visible",
        count _selected,
