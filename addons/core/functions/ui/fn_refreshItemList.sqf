...
private _filterElapsed = diag_tickTime - _started;
...
private _presentation = _display getVariable ["RACA_rowPresentation",createHashMap];
if (_rebuild) then {_presentation = createHashMap};
private _presentationRevision = [uiNamespace getVariable ["RACA_catalogGeneration",0],uiNamespace getVariable ["RACA_tagRevision",0],uiNamespace getVariable ["RACA_limitsRevision",0]];
private _presentationCache = uiNamespace getVariable ["RACA_catalogPresentation",createHashMap];
if ((_presentationCache getOrDefault ["revision",[]]) isNotEqualTo _presentationRevision) then {
    _presentationCache = createHashMapFromArray [["revision",_presentationRevision],["rows",createHashMap]];
    uiNamespace setVariable ["RACA_catalogPresentation",_presentationCache];
};
private _staticRows = _presentationCache get "rows";
...
private _focus = _display getVariable ["RACA_focusedClass",""];
{
    ...
    private _classKey = toLowerANSI _class;
    private _static = _staticRows getOrDefault [_classKey,[]];
    if (_static isEqualTo []) then {
        ...
        _static = [([_name,_class,_mod,"Tags: "+((_tags getOrDefault [_classKey,[]]) joinString ", ")] + _policies) joinString toString [10]];
        _staticRows set [_classKey,_static];
    };
    private _included = _selected getOrDefault [_class,false];
    private _favorite = _favorites getOrDefault [_class,false];
    private _isInherited = _inherited getOrDefault [_class,false];
    private _highlighted = _h getOrDefault [_class,false];
    private _rowState = [_included,_favorite,_isInherited,_highlighted,_static select 0];
    if (_rebuild || {_rowState isNotEqualTo (_presentation getOrDefault [_classKey,[]])}) then {
    _list lnbSetPicture [[_row,0],[RACA_TEXTURE_UNCHECKED,RACA_TEXTURE_CHECKED] select _included];
    private _color = if (_isInherited) then {[0.55,0.82,1,1]} else {if (_favorite) then {[1,0.82,0.35,1]} else {[1,1,1,1]}};
    { _list lnbSetColor [[_row,_x],_color] } forEach [1,2,3,4];
    private _tooltip = _static select 0;
    {_list lnbSetTooltip [[_row,_x],_tooltip]} forEach [0,1,2,3,4];
    _list lbSetSelected [_row,_highlighted];
    _presentation set [_classKey,_rowState];
    };
    if (_class isEqualTo _focus && {_rebuild}) then {_list lnbSetCurSelRow _row};
} forEach _slice;
_display setVariable ["RACA_renderKey",_renderKey];
_display setVariable ["RACA_rowPresentation",_presentation];
...
diag_log format ["[RACA][PERF] catalogue filter records=%1 matches=%2 changed=%3 seconds=%4",count _catalog,count _results,_newFilter,_filterElapsed];
diag_log format ["[RACA][PERF] catalogue render rows=%1 rebuild=%2 seconds=%3",count _slice,_rebuild,(diag_tickTime - _started) - _filterElapsed];
