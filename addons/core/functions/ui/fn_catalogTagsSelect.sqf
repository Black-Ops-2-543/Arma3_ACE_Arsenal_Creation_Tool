...
{_available set [_x select 1, true]} forEach (uiNamespace getVariable ["RACA_itemCatalog", []]);
private _availableCount = {_available getOrDefault [_x, false]} count _classes;
...
(_display displayCtrl RACA_IDC_CATALOG_TAG_REMOVE) ctrlEnable (_selectedCount > 0);
true
