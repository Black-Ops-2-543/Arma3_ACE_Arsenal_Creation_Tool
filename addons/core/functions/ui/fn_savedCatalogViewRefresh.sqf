...
private _restore = -1;
{
    _x params ["", "", "_name", "_search", "_category", "_source", "_addon", "_author", "_sortField", "_ascending"];
    private _sort = format ["%1 %2", toUpperANSI _sortField, ["Z-A", "A-Z"] select _ascending];
    private _row = _list lnbAddRow [_name, _search, _category, _source, _addon, _author, _sort];
    _list lnbSetData [[_row, 0], _name];
    private _tooltip = format ["Search: %1%7Category: %2%7Mod: %3%7Add-on: %4%7Author: %5%7Sort: %6", _search, _category, _source, _addon, _author, _sort, toString [10]];
    {_list lnbSetTooltip [[_row, _x], _tooltip]} forEach [0, 1, 2, 3, 4, 5, 6];
