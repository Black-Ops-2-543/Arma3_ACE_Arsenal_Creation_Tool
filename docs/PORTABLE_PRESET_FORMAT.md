...
Import text is never passed to `compile`, `compileFinal`, `call`, `spawn`, or `execVM`. There is no fixed item/token/character ceiling that substitutes for the old 20,000-item restriction. Named resource safeguards currently bound the clipboard input to 33,554,432 characters, one quoted literal to 4,096 characters, generic recovery to 1,000,000 scanned candidates, retained unavailable samples to 64, and displayed warning rows to 96. These are work/review limits rather than arsenal-size limits; the failure message names the exceeded resource and recommends JSON, a plain class list, or a narrowed migration source. Invalid data, cancellation, a newer superseding operation, concurrent profile changes, and engine resource failure leave the library untouched. The engine's native `fromJSON` call itself is not interruptible, but no profile write occurs until all later checkpoints and review have passed.
...
Copy the complete existing SQF file, optionally enter a desired preset name, and choose **Import Automatically**. A current RACA export is recognized by its versioned marker and required structural envelope, then only its literal `_arsenalItems` array is read. Other source enters the conservative lexer, which recognizes quoted string literals, doubled SQF quotes, `//` line comments, and `/* ... */` block comments. It scans bounded 65,536-character windows while preserving quote/comment state across boundaries, retains unique candidates instead of a full token corpus, and resolves them through the loaded catalogue index. Comment contents are ignored; source code is never compiled or executed. This recovers common category arrays combined with `+`, `append`, or `arrayIntersect` even though RACA does not evaluate variable order.
...
All Creator exports and copyable reports use the same Unicode-safe helper. It sends the exact text to the clipboard and also writes versioned, ordered RPT records with a decimal copy ID, code-point length, and order-sensitive float-safe `P23X2` digest. If clipboard contents are lost, use `tools\reconstruct-rpt-copy.ps1` with the RPT path and copy ID. The v3 reader rejects missing, duplicate, conflicting, reordered, or substituted chunks unless the exact length and dual digest agree. Existing v2 `P24X2` and v1 additive-checksum records remain readable when their original decimal envelopes are intact; v1 emits an explicit weaker-integrity warning.
