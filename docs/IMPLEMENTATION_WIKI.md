...
The runtime records events with severity and context. Authenticated server administrators can use an ACE self-interaction dashboard to inspect configured objects, live sessions, quota records, and recent audit events; issue requests; and copy audit evidence. Registry counters use revisioned per-object cached summaries that refresh when configurations, sessions, quotas, expiry, respawn, or disconnect state changes. Audit records use bounded cursor pages. Copy Audit freezes the authorized server's current retained range, requests every page from that snapshot, verifies stable totals/cursors, and expires abandoned snapshots after two minutes before producing the complete report. Authorization and page bounds are checked server-side, not merely by hiding a client button. The dashboard also provides guided listen-host/client/JIP rehearsal snapshots and probes for multiplayer evidence gathering.
